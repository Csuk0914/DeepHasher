...
def image_gen(stl_id, img_id, save_path, num_total):
    ...
    v_y = -normal_test
    v_y = v_y / np.linalg.norm(v_y)
    # get v_z use init v_x (randomize x and z elements)
    v_x_init = np.random.rand(2) * 2 - 1
    v_x_initb = -(v_y[0] * v_x_init[0] + v_y[2] * v_x_init[1]) / v_y[1]
    v_x = np.array([v_x_init[0], v_x_initb, v_x_init[1]])
...
stl_total = mesh_test.vectors.shape[0] # total number of mesh faces
img_idx = 0

if num_train_imgs!=0:
    label_train_all = np.zeros([num_train_imgs, label_length])
    while img_idx != num_train_imgs:
        stl_idx = floor(np.random.rand(1)*stl_total)
